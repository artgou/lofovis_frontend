...
export function Uploader(inputQName,
  ...
  listItemHtml = listItemHtml || `<li class="weui-uploader__file"><img src="{SRC}" alt="图片加载中。。。"><b class="del">x</b></li>`
  ...
  var maxSize = Int($input.data('maxsize') || 1024 * 1024 * 1024 * 5)
  ...
  if (!$listStats || !$listStats.data('enabled') || !$listStats.length) {
    $listStats = null
  }
  ...
  function addImgView(img) {
    // myFile = '/static/m/v1img/loading.gif'
    fileList.push(img)
    if (fileList.length >= max) {
      $input.parent().hide()
    }
    let $item
    'object' === typeof img && (img = URL.createObjectURL(img))
    if ($singleImg) {
      $singleImg.attr('src', img)
      $singleImg.find('.del').click(deleteImg)
    } else if ($list) {
      $item = $(listItemHtml.replace('{SRC}', img))
      $list.append($item)
  ...
  function resizeImage(imgFile, resizeWidth, resizeHeight, isSquare, callback) {
    if (imgFile.type.indexOf('image/') > -1) {
      ...
      if (isSquare) {
        ...
        let resizeWidth = 2048;
        image = new Image();
        let quality = imgFile.size > uploadMaxSize ? uploadMaxSize / imgFile.size : 1; //压缩系数0-1之间
        let oWidth, oHeight;
        image.onload = function() {
          oWidth = this.width;
          oHeight = this.height;
      ...
      // 非图片文件
      callback && callback('/static/m/v1img/credits/icon_file.png');
    }
  }
...
export function initUploaderView(domId, folder, uploadApi = '/m/upload') {
  let uploaderList = []
  $(domId).each(function() {
    let $this = $(this)
    let id = '#' + this.id
    let uploader = new Uploader(
      id + ' .weui-uploader__input',
      null,
      ...
      0,
      0,
      function(imgData, fuc) {
        const args = new FormData()
        args.append('folder', folder)
        args.append('base64', imgData)
        // let item = fuc()
        // aaa()
        Ajax('POST', uploadApi, args, ({
          errno,
          errmsg,
          data
        }) => {
          if (errno === 0) {
            fuc(data.url)
            // item.children('img').attr('src', data.url)
          }
        })
      }
    )
  ...
  for (let uploader of uploaderList) {
    ...
    if (imgFiles) {
      ...
      if (count < min) {
        ...
        uploaderImgs[name] = new Array()
        for (let i = 0, len = imgFiles.length; i < len; i++) {
          uploaderImgs[name].push(imgFiles[i])
          // uploaderImgs.push({
          //   key: `${name}_${i}`,
          //   val: imgFiles[i]
          // })
