...
var getWebpackConfig = function (config0) {
  ...
  let copyConfig = [];
  let copyFromPaths = ['static', 'staticDev']
  // let copyFrom = path.resolve(__dirname, '../', config.build.assetsDirectory);
  copyFromPaths.map(str => {
    let copyFrom = path.resolve(__dirname, '../', str);
  ...
  var webpackConfig = merge(baseWebpackConfig, {
    ...
    output: {
      ...
      filename: utils.assetsPath(config.build.assetsPage + '/[name].js'),
      chunkFilename: utils.assetsPath(config.build.assetsPage + '/[id].chunk.js?[chunkhash:10]')
    },
    plugins: [
      ...
      new ExtractTextPlugin({
        filename: utils.assetsPath(config.build.assetsPage + '/[name].css?[contenthash:10]')
      }),
